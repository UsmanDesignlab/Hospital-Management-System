...
function getCommonDetails(details, settings) {
    ...
    const defaultJsDoc = settings.supplyDefaultsInJsDoc && value !== undefined ? util.inspect(value, { depth: null }) : undefined;
...
function createAllowTypes(details, settings) {
    ...
    // at least one value
    if (values.length !== 0) {
        const allowedValues = values.map((value) => makeTypeContentChild({ content: typeof value === 'string' ? toStringLiteral(value, settings.doublequoteEscape) : `${value}` }));
